...
```{r}
taxa <- 
  purrr::map(checklists$datasetKey,
  ~rgbif::name_usage(datasetKey = ., limit = 99999),
  .progress = "taxa"
) %>%
  purrr::map_dfr("data")
...
1. Get verbatim information for our key from GBIF.

```{r}
verbatim_info <- purrr::map(
  taxa$taxonKey,
...
Extract distribution extension
...
distributions <-
  ...
  filter(
    countryCode == "BE",
    establishmentMeans %in% c("introduced", "Alien", "vagrant"),
    !occurrenceStatus %in% c("presence uncertain", "absent", "doubtful", "Uncertain", "Not specified", "excluded", "Recorded in error", "In captivity/cultivated", "Absent", "Extirpated", "Eradicated") # Inverse filter!
  )
```
...
taxa <-
  ...
  mutate(validDistribution = if_else(
    taxonKey %in% unique(distributions$key),
    TRUE,
    FALSE
  ))
```
...
1. Filter taxa on having valid distribution and `nubKey` (`bb_key`) and create vector of unique `nubKey`s.
...
progressr::with_progress({
  progress_bar <- progressor(steps = length(nub_keys))
  backbone_info <-
    future_map_dfr(
      nub_keys,
      function(x) {
...
* [`data/interim/taxa_with_verification.csv`](https://github.com/trias-project/unified-checklist/blob/master/data/interim/taxa_with_verification.csv): subset of checklist taxa with a valid distribution and an empty column `verificationKey`. This file will be used in later steps.
