...
progressr::with_progress({
  progress_bar <- progressor(steps = length(verification_keys))
  backbone_info <-
    future_map_dfr(
      verification_keys,
      function(x) {
...
## Explicitly remove incorrect taxa

1. Some taxa are purposely excluded in a source checklist (e.g. see [this issue](https://github.com/trias-project/unified-checklist/issues/32#issuecomment-621178197) for [Alien birds](https://doi.org/10.15468/wr3gis)), but still end up in the unified checklist because they are incorrectly included in another no-longer-updated source checklist (e.g. [RINSE pathways](https://doi.org/10.15468/guejza)). Here we explicitly remove those taxa:
...
taxa_unified <-
  taxa_unified %>% filter(
    ...
    scientificName != "Olios (Walckenaer, 1806)", # Don't include entire genus
    scientificName != "Pholcus Walckenaer, 1805" # Don't include entire genus
  )
```
