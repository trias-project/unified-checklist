...
3. Group taxa by `verificationKey` (saving `datasetKey` and `taxonKey` bundled per key).
...
taxa_unified <-
  ...
  summarize(
    ...
    taxonKey = paste(sort(unique(taxonKey)), collapse = ",")
  )
```
...
4. Join backbone information with our unified taxa, so we keep `datasetKey` and `taxonKey`.
...
5. Move columns `datasetKey` and `taxonKey` to the end and rename:

```{r unify_taxa-9}
taxa_unified <-
  taxa_unified %>%
  select(-datasetKey.x,everything()) %>%
  select(-taxonKey,everything()) %>% 
  select(-datasetKey.y) %>% 
  rename(datasetKey = datsetKey.x)
```
