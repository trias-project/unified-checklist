...
distributions_unified <-
  ...
  mutate(locality = recode(locality,
    ...
    "Kanaal van Gent naar Terneuzen" = "Flemish Region",
...
distributions_unified <-
  ...
  # Take earliest year, latest year and note taxonKey(s), scientificName(s), degree of establishment and pathways of introduction
  summarize(
    startYear = min(startYear, na.rm = TRUE),
    endYear = max(endYear, na.rm = TRUE),
    taxonKeys = paste(unique(taxonKey), collapse = "|"),
    scientificNames = paste(unique(scientificName), collapse = "|"),
    degreeOfEstablishments = paste(unique(degreeOfEstablishment), collapse = "|"),
...
distributions_unified <-
  ...
  summarize(
    ...
    degreeOfEstablishments = first(degreeOfEstablishments),
    pathways = first(pathways)
  ) %>%
  ungroup() %>%
