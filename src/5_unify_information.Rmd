...
3. Map `locality` and `locationId` to regional or national level.

```{r unify_information-5, echo = TRUE}
distributions_unified <-
  distributions_unified %>%
  
  # Map localities and locationId
  left_join(locations, by = c("locationId", "locality")) %>%
  select(-c(locality, locationId)) %>%
  rename(locationId = locationId_assigned, 
         locality = locality_assigned) %>%
  
  # Order columns
  select(names(distributions_unified))
```
...
distributions_unified <-
  ...
  mutate(startYear = ifelse(!is.infinite(startYear), 
                            as.integer(startYear), 
                            NA_integer_),
         endYear = ifelse(!is.infinite(endYear), 
                            as.integer(endYear), 
                            NA_integer_))
```
...
5. Add distribution at national level if not present

```{r unify_information-7}
# Find taxa without distribution at national level
distributions_no_belgium <-
  distributions_unified %>%
  group_by(verificationKey) %>%
  
  # assess presence of distribution at national level (Belgium)
  summarize(has_be_distr = "Belgium" %in% locality) %>%
  
  # filter taxa with regional distributions only
  filter(has_be_distr == FALSE) %>%
  left_join(distributions_unified,
             by = "verificationKey") %>%
  
  # remove locality related columns
  select(-c(locality, locationId, has_be_distr)) 

# Assign distribution at national level to these taxa
distributions_belgium_to_add <-
  distributions_no_belgium %>%
  
  # group by verificationKey
  group_by(verificationKey) %>%
  
  # Take earliest year, latest year:
  summarize(
    startYear = min(startYear, na.rm = TRUE),
    endYear = max(endYear, na.rm = TRUE)
  ) %>%
  
  # Add distribution at national level
  mutate(locality = "Belgium",
         locationId = "ISO_3166-2:BE") %>%
  
  # Join distribution with taxon to get verificationKey
  left_join(distributions_no_belgium %>%
              distinct(verificationKey, 
                       datasetKey, 
                       taxonKeys, 
                       scientificNames),
            by = "verificationKey")

# Add these new distributions
distributions_unified <-
  distributions_unified %>%
  bind_rows(distributions_belgium_to_add) %>%
  
  # Sort by verificationKey
  arrange(verificationKey)
```

6. Save to [CSV](https://github.com/trias-project/unified-checklist/blob/master/data/interim/distributions_unified.csv).

```{r unify_information-8}
write_csv(distributions_unified, 
