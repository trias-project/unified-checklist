...
## Create Belgian distributions where missing
distributions_unified_belgium_from_regions <-
  distributions_unified %>%
  
  # Group by taxonKey within a checklist 
  group_by(taxonKey) %>%
  
  # Filter taxonKey without national distribution
  filter(!"Belgium" %in% locality) %>%
  
  # Take earliest year, latest year from regional distributions 
  # and not source and remarks
  summarize(
...
## Add the just created Belgian distributions
distributions_unified <-
  distributions_unified %>%
  bind_rows(distributions_unified_belgium_from_regions) %>%
  
  # Sort by taxonKey
  arrange(taxonKey)
```

5. Choose a single distribution within a checklist for each location.
...
6. Choose a single distribution across checklists.
