...
The `datasetKey`s of the considered checklists are listed in `input_datasetKeys`. Since we want the DOI instead, we will separate this information into columns, gather to rows and join with the DOI, and spread and combine again into a single column.

```{r}
taxon <-
  taxon %>%
  
  # Separate input_datasetKeys on "," in as many columns as there are checklists
  separate(
    input_datasetKeys,
