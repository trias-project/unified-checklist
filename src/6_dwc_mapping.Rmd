...
```{r echo = TRUE}
taxon %<>% mutate(dwc_datasetName = "Unified checklist of alien species in Belgium")
```
...
Inspect differences between `genus` and `canonicalName_genus`:
...
The `genus` under which the GBIF Backbone Taxonomy has classified a taxon can differ from the genus in the scientific name. Since we consider all our taxa as accepted taxa, we take the genus from the scientific name (i.e. the `canonicalName_genus`).

```{r echo = TRUE}
taxon %<>% mutate(dwc_genus = canonicalName_genus)
```
...
```{r echo = TRUE}
taxon %<>% mutate(dwc_taxonRank = str_to_lower(rank))
```
