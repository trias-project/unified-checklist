...
1. Add prefix `input_` to all column names to avoid name clashes with Darwin Core terms.
...
To generate this full citation, we create a helper function `add_source_citation(df, new_field, dataset_info)`.

```{r echo = FALSE}
add_source_citation <- function(df, new_field, dataset_info,
                                dataset_key = "input_datasetKey",
                                taxon_key = "input_taxonKey",
                                scientific_name = "input_scientificName") {
  
  # Rename df[["dataset_key"]] to "datasetKey"
  rename(df, datasetKey = dataset_key) %>%
  ...
  # Build full citation in new_field (requires !! to bypass nse)
  mutate(!!new_field := paste0(
...
1. Create a dataframe from the unified taxa:
...
taxon %<>% add_source_citation(
  new_field = "bibliographicCitation",
...
Create a dataframe from the unified distributions:
...
Create a dataframe from the unified descriptions:
...
Create a dataframe from the unified species profiles:
