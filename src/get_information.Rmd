...
1. Read data from [`data/raw/taxa.csv`](https://github.com/trias-project/unified-checklist/blob/master/data/raw/taxa.csv).
...
1. Filter on taxa with valid distribution.
...
```{r get_information-get_distributions_from_gbif}
...
```{r get_information-get_speciesprofiles_from_gbif}
progress_bar <- progress_estimated(length(taxon_keys))
...
```{r get_information-get_descriptions_from_gbif}
progress_bar <- progress_estimated(length(taxon_keys))
