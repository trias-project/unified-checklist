...
```{r get_taxa-get_backbone_info_from_gbif}
...
4. Remove duplicates (caused by [this bug](https://github.com/ropensci/rgbif/issues/324)).
...
5. Rename `accepted` to `acceptedName`.
...
6. Add prefix `bb_` to column names:

```{r}
colnames(backbone_info) <- paste0("bb_", colnames(backbone_info))
```

7. Merge backbone information with checklist taxa.
...
Save to [CSV](https://github.com/trias-project/unified-checklist/blob/master/data/raw/taxa.csv).
