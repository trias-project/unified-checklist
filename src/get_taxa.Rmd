...
checklists <- checklist_keys %>%
  map(function(x) rgbif::datasets(uuid = x)) %>%
...
checklist_taxa <- checklists$datasetKey %>%
  map_df(function(x) name_usage(
    datasetKey = x,
...
checklist_taxa <- checklist_taxa %>% mutate(valid_distribution =
  map_lgl(checklist_taxa$key, function(x) {
...
```{r}
backbone_info <- backbone_info %>% distinct()
```
...
checklist_taxa %>%
  ...
  summarize(
    taxa = n(),
    valid_dist_in_backbone = sum(valid_distribution & !is.na(bb_key)),
    invalid_distribution = sum(!valid_distribution),
    not_in_backbone = sum(valid_distribution & is.na(bb_key))
  ) %>%
  adorn_totals("row")
```
