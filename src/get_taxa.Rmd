...
checklist_taxa <- map_df(checklists$datasetKey, function(x) name_usage(
    datasetKey = x,
...
checklist_taxa %<>%
  mutate(valid_distribution = map_lgl(checklist_taxa$key, function(x) {
...
```{r}
backbone_info %<>% distinct()
```

Rename `accepted` to `acceptedName`:

```{r}
backbone_info %<>% rename(acceptedName = accepted)
```
