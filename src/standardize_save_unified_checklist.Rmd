...
input_data_dir <- here("data","interim")
input_taxon <- here(input_data_dir, "unified_checklist_taxon_full.tsv")
input_distribution <- here(
  input_data_dir, "unified_checklist_distribution_not_standardized.tsv")
input_native_range <- here(
  input_data_dir, "unified_checklist_native_range_not_standardized.tsv")
input_pathway <- here(
  input_data_dir, "unified_checklist_pathways_not_standardized.tsv")
input_invasion_stage <- here(
  input_data_dir, "unified_checklist_invasion_stage_not_standardized.tsv")
input_species_profile <- here(
  input_data_dir, "unified_checklist_species_profile_not_standardized.tsv")
```
...
output_taxon <- here(
  output_data_dir, "unified_checklist_taxon.tsv")
output_distribution <- here(
  output_data_dir, "unified_checklist_distribution.tsv")
output_native_range <- here(
  output_data_dir, "unified_checklist_native_range.tsv")
output_pathway <- here(
  output_data_dir, "unified_checklist_pathways.tsv")
output_invasion_stage <- here(
  output_data_dir, "unified_checklist_invasion_stage.tsv")
output_species_profile <- here(
  output_data_dir, "unified_checklist_species_profile.tsv")
```

# Get data

We import the data related to taxa.

## Get taxon core

```{r get_taxa}
taxon_full <- read_tsv(input_taxon, na = "")
```

## Get distribution

```{r get_distribution}
distribution_not_st <- read_tsv(input_distribution, na = "")
```
...
```{r get_native_range}
native_range_not_st <- read_tsv(input_native_range, na = "")
```

### Get pathway

```{r get_pathway}
pathway_not_st <- read_tsv(input_pathway, na = "")
```

### Get invasion stage

```{r get_invasion_stage}
invasion_stage_not_st <- read_tsv(input_invasion_stage, na = "")
```

## Get species profile

```{r get_species_profile}
species_profile_not_st <- read_tsv(input_species_profile, na = "")
```
