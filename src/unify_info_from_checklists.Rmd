...
1. Retrieve all relevant information from the checklists.
...
```{r taxa_with_verification_key_path}
taxa_verified_file <- here("data", "interim", "taxa_after_verification.tsv")
```

Load taxa:

```{r load_taxa_with_verification_key}
taxa_verified <- read_tsv(taxa_verified_file)
```
...
```{r input_checklist_metadata_file_path}
...
```{r save_taxa_with_vks}
write_tsv(taxa_verified, 
          path = here("data", "interim", "unified_checklist_taxon_full.tsv"))
```
...
## Split `temporal` in distribution

The column `temporal` of `distribution` contains one or two datums:
...
# Couple information to verification key
...
We need also to link such information to the checklist it comes from. We do it by adding column `checklist_datasetKey` containing the unique checklist identifier assigned by GBIF:
...
Within the same checklist, we might still end up with multiple distributions per verification key because some taxa share the same verification key:
...
At this point no multiple distributions should be present:

```{r show_vks_with_multiple_distributions_after_time_info}
distribution %>% 
  group_by(verification_key) %>%
  count() %>%
  arrange(desc(n)) %>%
...
```{r save_distribution}
write_tsv(distribution, 
          path = here("data",
                      "interim", 
                      "unified_checklist_distribution_not_standardized.tsv"))
```
...
native_range_df <- native_range_df %>%
  ...
  filter(rank_checklist == min(rank_checklist)) %>%
  ungroup() %>%
  select(verification_key, native_range, 
         source, checklist_datasetKey, language)
```
...
pathways_df <- pathways_df %>%
  rowwise() %>%
  mutate(rank_checklist = 
           which(.data$checklist_datasetKey == 
                   checklist_metadata$datasetKey)) %>%
  ungroup() %>%
  distinct()
...
pathways_df <- pathways_df %>%
  ...
  filter(rank_checklist == min(rank_checklist)) %>%
  ungroup()
# get rid of NA at higher ranked checklist if info present at lower ranked checklist
pathways_df <- pathways_df %>%
  ...
  select(verification_key, pathway_level1, pathway_level2, 
         source, checklist_datasetKey, language)
```
...
invasion_stages_df <- description_tidy %>%
  ...
  filter(rank_checklist == min(rank_checklist)) %>%
  ungroup() %>%
  select(verification_key, invasion_stage, 
         source, checklist_datasetKey, language)
```
...
```{r save_native_range}
write_tsv(native_range_df, 
          path = here("data", 
                      "interim", 
                      "unified_checklist_native_range_not_standardized.tsv"))
```
...
```{r save_pathways}
write_tsv(pathways_df, 
          path = here("data", 
                      "interim", 
                      "unified_checklist_pathways_not_standardized.tsv"))
```

### Save invasion stages

Save to TSV:

```{r save_invasion_stage}
write_tsv(invasion_stages_df, 
          path = here("data", 
                      "interim", 
                      "unified_checklist_invasion_stage_not_standardized.tsv"))
```
...
write_tsv(species_profiles, 
          path = here("data", 
                      "interim", 
                      "unified_checklist_species_profile_not_standardized.tsv"))
```
