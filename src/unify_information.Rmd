...
distributions_unified <-
  ...
  mutate(startYear = as.integer(str_sub(startYear, 1, 4)), 
...
distributions_unified_t <-
  ...
  summarize(
    startYear = if_else(any(is.finite(startYear)),
                        min(startYear, na.rm = TRUE),
                        NA_real_),
    endYear = if_else(any(is.finite(endYear)),
                      max(endYear, na.rm = TRUE),
                      NA_real_),
