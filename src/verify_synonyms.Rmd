...
3. `verification_key` is not a synonym (is `ACCEPTED` or `DOUBTFUL`)

Summary of the issues found among `verification_key` which should be reported to the experts:

```{r show_invalid_vks_synonym}
invalid_vks <- trias::gbif_verify_keys(unique(verify$verified_taxa$verification_key)) %>%
...
```{r}
invalid_vks %>% 
  count(synonyms = (is_synonym == TRUE))
```
