...
In this document we report and update taxa that require manual verification.
...
```{r}
checklist_taxa <- checklist_taxa %>% filter(valid_distribution)
```

# Read verification file

This file is created/updated in previous runs of this document and manually verified by humans who add a `lump_key`.

```{r}
verification_file <- read_tsv(
  here("data", "interim", "verification_file.csv"),
  col_types = cols(
    bb_key = col_integer(),
    bb_acceptedKey = col_integer(),
    date_added = col_date(),
    lump_key = col_character()
  )
)
```
...
* Verification action: try to manually find a matching taxon in the GBIF backbone taxonomy and add its `key` to `lump_key`. The match could consist of multiple `key`s  for e.g. hybrid formulas.
...
* Verification action: If we agree with the lumping, copy `bb_acceptedKey` to `lump_key`. If we do not agree with the lumping, copy the taxon `bb_key` to `lump_key` (i.e. that taxon is considered its own entity).

In all other cases, we can accept the information returned by the GBIF backbone taxonomy (i.e. we consider the `bb_key` as the `lump_key`).

Add and set `lump_key`:

```{r}
checklist_taxa <- checklist_taxa %>% mutate(lump_key = case_when(
...
write_tsv(verify$verified_taxa, 
          path = here("data", "output", "verified_taxa.tsv"),
          na = "")
```
...
taxa_verified <- write_tsv(taxa, 
                           path = here("data", "interim", "taxa_after_verification.tsv"),
                           na = "")
```
